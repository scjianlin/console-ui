...
export default class GlobalValue {
  ...
  getGlobalNavs() {
    if (!this._cache_['globalNavs']) {
      // console.log("globalNas==>",this.checkNavItem({}, params => this.hasPermission(params)));
      const navs = []

      globals.config.globalNavs.forEach(nav => {
        // console.log("nav==>",this.checkNavItem(nav.items, params => this.hasPermission(params)));
  ...
  // sailor components 
  getSailorNavs() {
    if (!this._cache_['sailorNavs']) {
      const navs = []
      globals.config.sailorNavs.forEach(nav => {
        const filteredItems = nav.items.filter(item =>
          this.checkNavItem(item, params => this.hasPermission(params))
        )
      ...
      this._cache_['sailorNavs'] = navs
    }
    return this._cache_['sailorNavs']
  }
